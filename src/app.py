...
def run_llm(provider, model_name, messages, max_toxens=500):
    if provider == "OpenAI":
    ...
    if provider == "Google":
        ...
        return client.chat.completions.create(
            model=model_name,
            messages=messages,
            max_tokens=max_toxens,
        ).choices[0].message.content
    else: 
        return client.chat.completions.create(
            model=model_name,
            messages=messages,
            max_completion_tokens=max_toxens,
...
with st.sidebar:
    ...
    #Save the provider and model to session state
